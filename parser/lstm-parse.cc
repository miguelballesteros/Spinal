...
static map<int,string> compute_spines(unsigned sent_len, const vector<unsigned>& actions, const vector<string>& setOfActions, map<int,string>* pr = nullptr) {
  map<int,string> spines;
  map<int,string> r;
  map<int,string>& rels = (pr ? *pr : r);
  //for(unsigned i=0;i<sent_len;i++) { heads[i]=-1; rels[i]="ERROR"; }
  for (int i=0;i<sent_len;i++) spines[i]="*"; 
...
//      std::cout<<"SWAP"<<"\n";
      ...
      //rels[depi] = actionString;
        ...
        unsigned toki=stacki.back();


        size_t first_char_in_rel = actionString.find('(') + 1;
        size_t last_char_in_rel = actionString.rfind(')') - 1;
        string pspine = actionString.substr(first_char_in_rel, last_char_in_rel - first_char_in_rel + 1);
        spines[toki]+="+"+pspine;
...
vector<unsigned> log_prob_parser(ComputationGraph* hg,
  ...
  //cerr << endl;
    map<int,int> spinesi;
    for (int i=0;i<sent.size()+10;i++) spinesi[i]=-1000;

    map<int,string> spines;
    for (int i=0;i<sent.size()+10;i++) spines[i]="*";
    ...
    while(stack.size() > 2 || buffer.size() > 1) {
      ...
      // get relation embedding from action (TODO: convert to relation from action?) SPINAL: this also includes the NON-TERMINALS!! is it okay?
	...
	spinesi[toki]=action;
      ...
      last_actions.push_back(actionString);
    }
...
unsigned compute_correct(const map<int,int>& ref, const map<int,int>& hyp, const map<int,string>& refSpine, map<int,string>& hypSpine, unsigned len) {
  unsigned res = 0;
  for (unsigned i = 0; i < len; ++i) {
    auto ri = ref.find(i);
    auto hi = hyp.find(i);
    auto rs = refSpine.find(i);
    auto hs = hypSpine.find(i);

    assert(ri != ref.end());
    assert(hi != hyp.end());
    if (ri->second == hi->second && rs->second == hs -> second) ++res;
  }
  return res;
}
...
void output_conll(const vector<unsigned>& sentence, const vector<unsigned>& pos,
                  ...
                  const map<int,int>& hyp, const map<int,string>& rel_hyp, map<int,string> hypSpine) {
...
int main(int argc, char** argv) {
    ...
    while(!requested_stop) {
      ...
      if (logc % 25 == 1) { // report on dev set
	   ...
	   double lp = 0;
           ...
           map<int,string> refSpine = parser.compute_spines(sentence.size(), actions, corpus.actions, &rel_ref);
           map<int,string> hypSpine = parser.compute_spines(sentence.size(), pred, corpus.actions, &rel_hyp);

           correct_heads += compute_correct(ref, hyp, refSpine, hypSpine, sentence.size() - 1);
  ...
  if (true) { // do test evaluation
	...
	cout<<corpus.actions[pred[n]]<<"\n";
     }
     cout<<"\n";

      output_conll(sentence, sentencePos, sentenceUnkStr, corpus.intToWords, corpus.intToPos, hyp, rel_hyp, hypSpine);
